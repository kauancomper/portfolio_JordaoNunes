document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadInstagramFeed() {
        ...
        try {
            const response = await fetch(`https://graph.instagram.com/me/media?fields=id,caption,media_type,media_url,permalink,timestamp&access_token=${INSTAGRAM_TOKEN}`);
            const json = await response.json();
            allInstagramPosts = json.data
    ...
    function categorizePost(caption) {
        ...
        if (text.includes('social') || text.includes('evento') || text.includes('projeto') || text.includes('parceria')) return 'social';
        return 'nature';
    }
